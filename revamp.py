...
def run(cfg: DictConfig) -> None:
    ...
    for i in range(len(passes)):
        ...
        print("[Start] ",clean_renders_preds)
        ...
        asr_script_path = "/home/wmy/project/Physical-Attacks-in-Embodied-Nav/test-patch/ASR.py"
        asr_command = f"python {asr_script_path}"
        print("[Start] Calling ASR script...")
        subprocess.run(asr_command, shell=True, check=True)
        # run()

    # process logfile
    process_logs = f"python src/results.py -i {cfg.sysconfig.log_dir}/revamp.log"
    print("[Start] ",process_logs)                  
